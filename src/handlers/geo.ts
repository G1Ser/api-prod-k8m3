...
const searchDivisions = async (sql: Sql, keyword: string, countryCode?: string) => {
  // 1) 命中 adm2：直接返回前 5 条
  const adm2Matches = await findByKeywordAndLevel(sql, keyword, 'ADM2', 5, countryCode);
  ...
  const adm1Matches = await findByKeywordAndLevel(sql, keyword, 'ADM1', 1, countryCode);
  ...
  const adm0Matches = await findByKeywordAndLevel(sql, keyword, 'ADM0', 1, countryCode);
  if (adm0Matches.length > 0) {
    const adm0 = adm0Matches[0];
    const adm1List = await findChildrenByParent(sql, 'adm0_id', adm0.id, 'ADM1', 5);
    ...
    return adm1List.map(adm1 => formatJSON(cityMap.get(adm1.main_id)!, adm1, adm0));
  }
  return [];
};

export const handleAdcodeSearch = async (request: Request, env: Env, origin: string) => {
  const url = new URL(request.url);
  const keyword = url.searchParams.get('keyword')?.trim() ?? '';
  if (!keyword) {
    return createErrorResponse('Missing Parameters', "'keyword' is required.", 400);
  }
...
export const handleCoordinateSearch = async (request: Request, env: Env, origin: string) => {
  const url = new URL(request.url);
  const keyword = url.searchParams.get('keyword')?.trim() ?? '';
  if (!keyword) {
    return createErrorResponse('Missing Parameters', "'keyword' is required.", 400);
